...
void s21_print_error(const char *pname, const char *str) {
  fprintf(stderr, "%s: %s\n", pname, str);
}
...
void s21_push_to_string_array(char ***array, const char *element, int *num) {
  int num_of_els = *num;
  if (num_of_els == 0) {
    *array = (char **)s21_calloc(sizeof(char *));
  } else {
    *array = (char **)s21_realloc(
        *array, (unsigned long)(num_of_els + 1) * sizeof(char *));
  }
  (*array)[num_of_els] = (char *)s21_calloc((strlen(element) + 1));
...
char *s21_pull_from_string_array(char ***array, int *num) {
  ...
  if (*num > 0) {
    ...
    for (int i = 0; i < *num - 1; i++) {
      (*array)[i] = s21_realloc((*array)[i],
                                sizeof(char *) * (strlen((*array)[i + 1]) + 1));
